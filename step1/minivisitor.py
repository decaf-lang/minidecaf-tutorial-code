...
# 给 ASTNode 实现 accept 方法，一种不太好的写法
miniparser.ASTNode.accept = accept


class Visitor:
    """默认行为是遍历、但什么也不做"""
    def visitProgram(self, node:miniparser.ASTNode):
        node.children[0].accept(self) # func

    def visitFunction(self, node:miniparser.ASTNode):
        node.children[0].accept(self) # ty
        node.children[5].accept(self) # stmt

    def visitType(self, node:miniparser.ASTNode):
        pass

    def visitStatement(self, node:miniparser.ASTNode):
        node.children[1].accept(self) # expr

    def visitExpression(self, node:miniparser.ASTNode):
        pass
...
class TargetCodeEmission(Visitor):
    def visitFunction(self, node:miniparser.ASTNode):
        ...
        if ident.text != "main":
            raise Exception(f"function name: 'main' expected, '{ident.text}' found")
